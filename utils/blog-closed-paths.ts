/** José Excel v2 — signed 31-path deny list. Trailing slashes normalize to these. */
export const CLOSED_BLOG_PATHS: readonly string[] = Object.freeze([
...
export function isClosedBlogPath(pathname: string): boolean {
  return CLOSED_BLOG_PATH_SET.has(normalizeBlogPath(pathname));
}

export function isClosedBlogPost(post: {
  slug: string;
...
export function filterOpenBlogPosts<T extends {
  slug?: string;
  href?: string;
  categories?: Array<{ slug: string }>;
}>(posts: T[]): T[] {
