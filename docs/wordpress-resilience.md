...
- Hard validation budget: **90 seconds**, enforced by `npm run build:guarded`. A timeout must fail validation; it must not be “fixed” by raising concurrency without new load evidence.
